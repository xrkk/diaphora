...
class CChooser():
    ...
    def add_item(self, item):
        if self.title.startswith("Unmatched in"):
            self.items.append(["%05lu" % self.n, "%08x" %
                               int(item.ea), item.vfname])
        else:
            self.items.append(["%05lu" % self.n, "%08x" % int(item.ea), item.vfname,
                               "%08x" % int(
                                   item.ea2), item.vfname2, "%.3f" % item.ratio,
...
class CBinDiff:
    def __init__(self, db_name, chooser=CChooser):
        ...
        self.open_db()
        self.matched1 = set()  # names in p1, 所有类型的匹配的
        self.matched2 = set()  # names in p2, 所有类型的匹配的
        self.matches_cache = {}
        ...
        self.max_processed_rows = self.get_value_for(
            "MAX_PROCESSED_ROWS", MAX_PROCESSED_ROWS)
    ...
    def create_schema(self):
        ...
        row = cur.fetchone()
        if not row:
            cur.execute("insert into main.version values ('%s')" %
                        VERSION_VALUE)
            ...
            for key in bb_data:
                for insn in bb_data[key]:
                    ...
                    if db_id is None:
                        ...
                        cur_execute(sql, (str(addr), mnem, disasm, cmt1,
                                          cmt2, name, mtype, pseudocomment, pseudoitp))
                        db_id = cur.lastrowid
    ...
    def find_equal_matches_parallel(self):
        ...
        if len(rows) > 0:
            for row in rows:
                ...
                self.best_chooser.add_item(CChooser.Item(
                    ea, name, ea, name, "100% equal", 1, nodes, nodes))
    ...
    def run_heuristics_for_category(self, arg_category):
        ...
        if self.hooks is not None:
            if 'get_heuristics' in dir(self.hooks):
                heuristics = self.hooks.get_heuristics(
                    arg_category, heuristics)
        ...
        for heur in heuristics:
            ...
            if ratio == HEUR_TYPE_NO_FPS:
                t = Thread(target=self.add_matches_from_query,
                           args=(sql, best))
            elif ratio == HEUR_TYPE_RATIO:
                t = Thread(target=self.add_matches_from_query_ratio,
                           args=(sql, best, partial))
            elif ratio == HEUR_TYPE_RATIO_MAX:
                t = Thread(target=self.add_matches_from_query_ratio_max,
                           args=(sql, min_value))
            else:
            ...
            while len(threads_list) >= total_cpus:
                for i, t in enumerate(threads_list):
                    if not t.is_alive():
                        debug_refresh("[Parallel] Heuristic '%s' took %f..." % (
                            t.name, time.time() - t.time))
                        del threads_list[i]
                        debug_refresh(
        ...
        if len(threads_list) > 0:
            log_refresh("[Parallel] Waiting for remaining %d thread(s) to finish..." % len(
                threads_list), do_log=False)
            ...
            while len(threads_list) > 0 and not do_cancel:
                ...
                for i, t in enumerate(threads_list):
                    ...
                    if not t.is_alive():
                        ...
                        del threads_list[i]
                        debug_refresh(
                            "[Parallel] Waiting for remaining %d thread(s) to finish..." % len(threads_list))
                        break
    ...
    def check_ratio(self, ast1, ast2, pseudo1, pseudo2, asm1, asm2, md1, md2):
        """
        ast: pseudocode_primes: 伪代码素数
        pseudo: pseudocode: 伪代码字符块
        asm: assembly: 反汇编字符块
        md: md_index:
    ...
    def add_matches_from_query_ratio(self, sql, best, partial, unreliable=None, debug=False):
        ...
        while self.max_processed_rows == 0 or (self.max_processed_rows != 0 and i < self.max_processed_rows):
            ...
            r = self.check_ratio(ast1, ast2, pseudo1,
                                 pseudo2, asm1, asm2, md1, md2)
            if debug:
            ...
            if self.hooks is not None:
                if 'on_match' in dir(self.hooks):
                    ...
                    d2 = {"ea": ea, "bb": bb2, "name": name2, "ast": ast2,
            ...
            if r == 1.0:
                self.best_chooser.add_item(CChooser.Item(
                    ea, name1, ea2, name2, desc, r, bb1, bb2))
                self.matched1.add(name1)
                self.matched2.add(name2)
            elif r >= 0.5:
                partial.add_item(CChooser.Item(
                    ea, name1, ea2, name2, desc, r, bb1, bb2))
                self.matched1.add(name1)
                self.matched2.add(name2)
            elif r < 0.5 and unreliable is not None:
                unreliable.add_item(CChooser.Item(
                    ea, name1, ea2, name2, desc, r, bb1, bb2))
                self.matched1.add(name1)
                self.matched2.add(name2)
            else:
    ...
    def add_matches_from_query(self, sql, choose):
        ...
        while 1:
            ...
            choose.add_item(CChooser.Item(
                ea, name1, ea2, name2, desc, 1, bb1, bb2))
            self.matched1.add(name1)
            self.matched2.add(name2)
        cur.close()
    ...
    def search_small_differences(self, choose):
        """
        1. 相同的: nodes/edges/mnemonics/cyclomatic_complexity, 且func_1.names不为空
        2. names共同个数 / names最大个数 >= 0.5
          - ratio == 1: -> best
          - ratio < 1: -> partial
        ."""
        cur = self.db_cursor()
            ...
            if ratio >= 0.5:
                ...
                if self.hooks is not None:
                    if 'on_match' in dir(self.hooks):
                        ...
                        d2 = {"ea": ea, "bb": bb2, "name": name2, "ast": ast2,
                              "pseudo": pseudo2, "asm": asm2, "md": md2}
                        should_add, r = self.hooks.on_match(
                            d1, d2, desc, ratio)
    ...
    def find_same_name(self, choose):
        ...
        if len(rows) > 0 and not self.all_functions_matched():
            for row in rows:
                ...
                ratio = self.check_ratio(
                    ast1, ast2, pseudo1, pseudo2, asm1, asm2, md1, md2)
                ...
                if float(ratio) == 1.0 or (md1 != 0 and md1 == md2):
                    self.best_chooser.add_item(CChooser.Item(
                        ea, name, ea2, name, desc, 1, bb1, bb2))
                else:
                    choose.add_item(CChooser.Item(
                        ea, name, ea2, name, desc, ratio, bb1, bb2))
    ...
    def find_matches_in_hole(self, last, item, row):
        """
        last: p1中一段不匹配函数(`hole`)的上沿
        item: p1中一段不匹配函数(`hole`)的下沿
        ...
        try:
            ...
            # 设置ratio的最小值: 查找到的匹配度只要比 item 匹配度高, 或者比 0.6 高, 就可以算新的匹配
            thresold = min(0.6, float(item[5]))
            for j in range(0, min(10, id1 - last)):  # 双层循环, 每层最多10次, 共最多100次
                for i in range(0, min(10, id1 - last)):
                    cur.execute(sql, (id1+j, id2+i))  # 这里应该是 - ??
                    rows = cur.fetchall()
                    if len(rows) == 2:
                        name1 = rows[0]["name"]
                        name2 = rows[1]["name"]
                        ...
                        if r > thresold:
                            ...
                            if r == 1:
                                ...
                                self.partial_chooser.add_item(CChooser.Item(
                                    ea, name1, ea2, name2, desc, r, bb1, bb2))
                                self.matched1.add(name1)
                                self.matched2.add(name2)
                            else:
                                self.unreliable_chooser.add_item(CChooser.Item(
                                    ea, name1, ea2, name2, desc, r, bb1, bb2))
                                self.matched1.add(name1)
                                self.matched2.add(name2)
        finally:
    ...
    def find_from_matches(self, the_items):
        """
        p1的所有函数, 按idd升序排列, 组成了个线性数字
        把数组中已经包括在 the_items 里头的着色
        剩下没着色的: 只要是连续的, 就构成一个`hole`
        传入函数 self.find_matches_in_hole(last, item, row) 里的参数:
          - last: `hole`的顶部匹配项的func_1.idd
          - item: `hole`的底部的匹配项. 在子函数中只使用到了ratio值
          - row: 跟item是同一条匹配项, 但没ratio
        1. 把 the_items 里头的每个 match 按照 func_1.address 升序排序
        2.
        ...
        try:
            ...
            cur.execute(
                "create temporary table best_matches (id, id1, ea1, name1, id2, ea2, name2)")

            # Insert each matched function into the temporary table
            for i, match in enumerate(the_items):
                ea1 = match[1]
                name1 = match[2]
                ea2 = match[3]
                name2 = match[4]
                ...
                sql = """insert into best_matches (id, id1, ea1, name1, id2, ea2, name2)
                                   values (?, ?, ?, ?, ?, ?, ?)"""
                cur.execute(
                    sql, (i, id1, str(ea1), name1, id2, str(ea2), name2))
    ...
    def find_callgraph_matches_from(self, the_items, min_value):
        ...
        对于每个匹配项 item:
        - 只搞1次
        - 每次分2批: 先搞caller, 再搞callee
          - 找到p1里头func_1的所有caller/callee, 记作: func_1_xref_list
          - 找到p2里头func_2的所有caller/callee, 记作: func_2_xref_list
          - 两两比较, md_index相差绝对值小于1的, 构成新的pair
          - 计算新pair的ratio, 达到min_value, 就算一对新的match
        .
        """
        ...
        while len(the_items) > 0:
            ...
            if total_dones % 1000 == 0:
                ...
                log(fmt % (len(the_items), curr_best_matches,
            ...
            for call_type in ['caller', 'callee']:
                ...
                if len(main_address_set) > 0 and len(diff_address_set) > 0:
                    ...
                    cur.execute(sql % (
                        ("%s of %s/%s" % (call_type, tname1, tname2)),  # description
                        # f.address in (%s)
                        ",".join(main_address_set),
                        ",".join(diff_address_set)))                    # df.address in (%s)
                    matches = self.add_matches_from_cursor_ratio_max(
                        cur, self.partial_chooser, None, min_value)
    ...
    def find_brute_force(self):
        """
        1. 将2个程序中所有没匹配到的函数地址, 存到新表, 用 1/0 标识是哪个程序
        2. 从3个表中联合选: ...

        - func.md_index 过滤(>1) 后取交, 得: md_index_intersect
          - 遍历每个 md_index_x:
            - 得到: func_1_list. 遍历每个 func_1:
              - 得到: func_2_list, => 得到多个 func_1->func_2 匹配对 => 删除在match列表中已经存在的匹配对
        - func.kgh_hash 过滤(>7) 后取交....
        """
        cur = self.db_cursor()
        ...
        sql = """select distinct f.address ea, f.name name1, df.address ea2, df.name name2,
              ...
              where
              (
                (f.address = um.address and um.main = 1)
                or
                (df.address = um.address and um.main = 0)
              )
              and
              (
                (f.md_index = df.md_index and f.md_index > 1 and df.md_index > 1)
                or
                (f.kgh_hash = df.kgh_hash and f.kgh_hash > 7 and df.kgh_hash > 7)
              )"""
        cur.execute(sql)
        log_refresh("Finding via brute-forcing...")
        self.add_matches_from_cursor_ratio_max(
            cur, self.unreliable_chooser, None, 0.5)
    ...
    def create_choosers(self):
        ...
        self.unmatched_second = self.chooser(
            "Unmatched in secondary", self, False)  # p1中所有在p2里头没有匹配的函数
        self.unmatched_primary = self.chooser(
            "Unmatched in primary", self, False)   # p2中所有在p1里头没有匹配的函数
    ...
    def save_results(self, filename):
        ...
        try:
            ...
            cur.execute(sql, (self.db_name, self.last_diff_db,
                              VERSION_VALUE, time.asctime()))
    ...
    def diff(self, db):
        ...
        if row["value"] != VERSION_VALUE:
            Warning("The database is from a different version (current %s, database %s)!" % (
                VERSION_VALUE, row[0]))
            return False
